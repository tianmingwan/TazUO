...
public class TazUOChatWindow : MyraControl
{
    ...
    public TazUOChatWindow() : base(Language.Instance.TazUOChat.Title)
    {
        CanBeSaved = true;
    ...
    private void RebuildContent()
    {
        ...
        if (!_manager.IsConnected)
        {
            ...
            row.Widgets.Add(new MyraLabel(Language.Instance.TazUOChat.NotConnected, MyraLabel.TextStyle.P));
            row.Widgets.Add(new MyraButton(Language.Instance.TazUOChat.TryToConnect, () =>
            {
                _manager.Dispose();
                _manager.Init();
            }));
    ...
    private Widget BuildMainArea()
    {
        ...
        var channelScroll = new ScrollViewer { Height = PANEL_HEIGHT, Content = _channelsPanel};
        ...
        var userScroll    = new ScrollViewer { Height = PANEL_HEIGHT, Content = _usersPanel  };
    ...
    private Widget BuildInputRow()
    {
        ...
        {
            HintText = Language.Instance.TazUOChat.TypeAMessage,
        ...
        row.Widgets.Add(new MyraButton(Language.Instance.TazUOChat.Send, TrySend));
        row.Widgets.Add(new MyraButton(Language.Instance.TazUOChat.Options, () =>
        {
            ShowContextMenu((ContextMenuLabelToggle(!ProfileManager.CurrentProfile.DisableConnectToIrcOnLogin, Language.Instance.TazUOChat.AutoConnect), () =>
            {
    ...
    private void RebuildChannels()
    {
        ...
        _channelsPanel.Widgets.Add(new MyraLabel(Language.Instance.TazUOChat.Channels, MyraLabel.TextStyle.P));
        ...
        foreach (string ch in channels)
        {
            ...
            { Tooltip = Language.Instance.TazUOChat.LeaveChannel }));

            _channelsPanel.Widgets.Add(chRow);
        }

        // Join input row
        var joinBox = new MyraInputBox { HintText = Language.Instance.TazUOChat.ChannelHint, Width = CHANNEL_WIDTH - 30 };
        ...
        joinRow.Widgets.Add(new MyraButton("+", () => DoJoin(joinBox))
            { Tooltip = Language.Instance.TazUOChat.JoinTooltip });
    ...
    private void RebuildMessages()
    {
        ...
        if (!string.IsNullOrEmpty(_selectedChannel))
        {
            ...
            if (messages.Length == 0)
                _messagesPanel.Widgets.Add(new MyraLabel(Language.Instance.TazUOChat.NoMessagesYet, MyraLabel.TextStyle.P));
            else
        ...
        {
            _messagesPanel.Widgets.Add(new MyraLabel(Language.Instance.TazUOChat.SelectAChannel, MyraLabel.TextStyle.P));
        }
    ...
    private void RebuildUsers()
    {
        ...
        if (!string.IsNullOrEmpty(_selectedChannel))
        {
            ...
            _usersPanel.Widgets.Add(new MyraLabel(string.Format(Language.Instance.TazUOChat.UsersFormat, users.Length), MyraLabel.TextStyle.P));
            foreach (string user in users)
                _usersPanel.Widgets.Add(new MyraLabel(user == _manager.CurrentNick ? $"/c[green]{user}" : user, MyraLabel.TextStyle.P));
        }
        else
        {
            _usersPanel.Widgets.Add(new MyraLabel(Language.Instance.TazUOChat.Users, MyraLabel.TextStyle.P));
        }
